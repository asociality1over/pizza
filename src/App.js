    ...
    <div className="dark:bg-gray-800">
      ...
      <div className="mb-36 container xl:mx-auto md:mx-auto xl:w-1/6 md:w-3/6 sm:w-full flex flex-col sm:mx-3">
          <div className="xl:mt-48 mt-32 mb-6">
              ...
              <input value={email} onChange={emailHandler} onBlur={blurHandler} type="email" id="required-email" class="border-transparent flex-1 appearance-none border-b border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 dark:text-gray-100 dark:placeholder-gray-100 dark:bg-gray-800 shadow-sm text-base focus:outline-none focus:border-indigo-600 focus:border-2 focus:placeholder-white" name="email" placeholder="Ваш адрес электронной почты"/>
              {(emailDirty && emailError) && <p class="text-sm pl-4 mt-1 text-red-500">{emailError}</p>}
              
              
              <input value={password} onChange={passwordHandler} onBlur={blurHandler} type="password" id="password" class="mt-8 border-transparent flex-1 appearance-none border-b border-gray-300 w-full py-2 px-4 bg-white text-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-100 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:border-indigo-600 focus:border-2 focus:placeholder-white" name="password" placeholder="Ваш пароль"/>
      ...
      <Header />
        <div className="mb-36 container xl:mx-auto md:mx-auto xl:w-1/6 md:w-3/6 sm:w-full flex flex-col sm:mx-3">
            <div className="xl:mt-48 mt-32 mb-6">
                ...
                <input onChange={emailHandler} value={email} onBlur={blurHandler} type="email" id="required-email" class="border-transparent flex-1 appearance-none border-b border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 dark:text-gray-100 dark:placeholder-gray-100 dark:bg-gray-800 shadow-sm text-base focus:outline-none focus:border-indigo-600 focus:border-2 focus:placeholder-white" name="email" placeholder="Ваш адрес электронной почты"/> 
                {(emailDirty && emailError) && <p class="text-sm pl-4 mt-1 text-red-500">{emailError}</p>}
                <input onChange={passwordHandler} value={password} onBlur={blurHandler} type="password" id="password" class="mt-8 border-transparent flex-1 appearance-none border-b border-gray-300 w-full py-2 px-4 bg-white text-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-100 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:border-indigo-600 focus:border-2 focus:placeholder-white" name="password" placeholder="Ваш пароль"/> 
                {(passwordDirty && passwordError) && <p class="text-sm pl-4 mt-1 text-red-500">{passwordError}</p>}
                <input onChange={confirmPasswordHandler} value={confirmPassword} onBlur={blurHandler} type="password" id="confirmPassword" class="mt-8 border-transparent flex-1 appearance-none border-b border-gray-300 w-full py-2 px-4 bg-white text-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder-gray-100 placeholder-gray-400 shadow-sm text-base focus:outline-none focus:border-indigo-600 focus:border-2 focus:placeholder-white" name="confirmPassword" placeholder="Подтвердите пароль"/>             
                {(confirmPasswordDirty && confirmPasswordError) && <p class="text-sm pl-4 mt-1 text-red-500">{confirmPasswordError}</p>}        
                <div className="flex mt-16 items-center justify-between">

                    <a href="#" className="text-indigo-600">Уже есть аккаунт?</a>
                    
                  <button disabled={!formValid} class={formValid ? "inline-flex items-center bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600 rounded text-base mt-4 p-5 md:mt-0 text-white shadow-xl" : "inline-flex items-center bg-indigo-100 border-0 py-2 px-6 focus:outline-none rounded text-base mt-4 p-5 md:mt-0 text-white cursor-default"}>Регистрация
                    <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" class="w-4 h-4 ml-1" viewBox="0 0 24 24">
                    <path d="M5 12h14M12 5l7 7-7 7"></path>
                    </svg>
                  </button> 
